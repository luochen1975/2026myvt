self.dedup_mode = dedup_mode
    ...
    def load_blacklist(self, path: str) -> None:
        """加载黑名单文件"""
@@ -34,7 +38,6 @@ def merge(self, channels: list[Channel]) -> list[Channel]:
...
@@ -44,13 +47,11 @@ def apply_template(self, channels: list[Channel], template_path: str) -> dict:
                    if not line:
                        continue
                    if line.startswith('#'):
...
@@ -76,29 +77,54 @@ def apply_template(self, channels: list[Channel], template_path: str) -> dict:
