...
def _get_speed(c: Channel) -> float:
    """安全获取速度值，None 时返回 -1 确保排在最后"""
    return c.speed if c.speed is not None else -1


def _is_mobile(c: Channel) -> bool:
    """更宽松的移动源判断：检查 isp 字段或名称关键词"""
    isp = (c.extra.get("isp") or "").lower()
    name = c.name.lower()
    return isp == "mobile" or "移动" in name or "cmcc" in name or "migu" in name
...
class M3UExporter:
    ...
    def export_mobile_first(channels: List[Channel], output_file: str, epg_url: str = ""):
        """移动源优先排序导出"""
        mobile = [c for c in channels if _is_mobile(c)]
        other = [c for c in channels if not _is_mobile(c)]

        mobile.sort(key=_get_speed, reverse=True)
        other.sort(key=_get_speed, reverse=True)
...
class TXTExporter:
    ...
    def export(channels: List[Channel], output_file: str, include_speed: bool = False):
        ...
        for ch in channels:
            ...
            if include_speed and ch.speed is not None and ch.speed > 0:
...
class LogExporter:
    ...
    def export_speed_log(channels: List[Channel], log_file: str):
        ...
        sorted_ch = sorted(channels, key=_get_speed, reverse=True)
        for ch in sorted_ch:
            status = "✓" if (ch.speed is not None and ch.speed > 50) else "✗"
            isp = ch.extra.get("isp", "other") or "other"
            is_multicast = "组播" if ch.url.strip().lower().startswith(("udp://", "rtp://", "rtsp://")) else "单播"
            speed_str = f"{ch.speed:>10.1f}" if ch.speed is not None else "       N/A"
            lines.append(
                f"{status} {ch.name:<20} {ch.group:<15} {isp:<8} {is_multicast:<6} {speed_str}  {ch.url[:60]}"
            )
